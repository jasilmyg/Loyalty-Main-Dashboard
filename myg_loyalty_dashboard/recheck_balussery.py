...
if 'Branch' in df.columns:
    df_Balussery = df[df['Branch'].astype(str).str.contains('Balussery', case=False, na=False)].copy()
if 'Date' in df.columns:
    df['Customer Mobile'] = df['Customer Mobile'].astype(str).str.strip().str.replace('\.0$', '', regex=True)
    df_Balussery['ParsedDate'] = pd.to_datetime(df_Balussery['Date'], dayfirst=True, errors='coerce')
    df_Balussery = df_Balussery[df_Balussery['ParsedDate'].dt.date.astype(str).isin(['2026-06-27', '2026-06-28'])]
...
with connection.cursor() as cur:
    for date_str in ['2026-06-27', '2026-06-28']:
        day_df = df_Balussery[df_Balussery['ParsedDate'].dt.date.astype(str) == date_str]
