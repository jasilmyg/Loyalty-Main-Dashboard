...
file_path = r"C:\Users\jasil_myg\Desktop\myG Loyalty Main Dashboard\myg_loyalty_dashboard\DSR JUNE 2026 PART X.xlsx"
df = pd.read_excel(file_path, engine='calamine')
original_count = len(df)
print(f"Loaded {original_count} rows from Excel.")
print(f"Excel columns: {df.columns.tolist()}")

# --- STEP 2.5: Filter dates -----------------------------------------------------
print("\n" + "=" * 60)
print("STEP 2.5: Filtering Dates between 20/06/2026 to 26/06/2026...")
if 'Date' in df.columns:
    df['Date'] = pd.to_datetime(df['Date'], dayfirst=True, errors='coerce')
    # Filter for dates between 20th June 2026 and 26th June 2026
    start_date = pd.to_datetime('2026-06-20')
    end_date = pd.to_datetime('2026-06-26')
    
    date_mask = (df['Date'] >= start_date) & (df['Date'] <= end_date)
    df = df[date_mask].copy()
    print(f"  -> Rows after date filtering: {len(df)}")
    
    # We should also populate parsed_date if the db column exists
    if 'parsed_date' in db_cols:
        df['parsed_date'] = df['Date'].dt.date
else:
    print("  ! Error: 'Date' column not found in Excel!")
    sys.exit(1)
...
bad_mask = smc_mask.values | branch_mask.values
...
if 'Invoice Number' in matched_cols:
    ...
    if excel_invoices:
        with connection.cursor() as cur:
            # We can use ANY array syntax which is much faster than massive IN clause
            cur.execute('SELECT "Invoice Number" FROM sales_data WHERE "Invoice Number" = ANY(%s)', [excel_invoices])
...
import subprocess
# Let's run fast refresh or standard refresh
