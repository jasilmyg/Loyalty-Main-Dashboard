...
def generate_forecast():
    ...
    FY_END = pd.to_datetime("2027-03-31")
    ...
    query_daily = """
        ...
        FROM sales_data 
        WHERE "Date" LIKE '%%-04-2026' OR "Date" LIKE '%%-05-2026' OR "Date" LIKE '%%-06-2026'
        GROUP BY "Date"
    """
    ...
    X_future = np.arange(len(y_daily), len(y_daily) + remaining_days).reshape(-1, 1)
    
    # 3.1. Deep Learning (MLP Proxy for LSTM)
    mlp = MLPRegressor(hidden_layer_sizes=(100, 50), max_iter=500, random_state=42)
    mlp.fit(X, y_daily)
    pred_mlp = mlp.predict(X_future)
    
    # 3.2. Gradient Boosting (XGBoost Proxy)
    gbr = GradientBoostingRegressor(n_estimators=100, random_state=42)
    gbr.fit(X, y_daily)
    pred_gbr = gbr.predict(X_future)
    
    # 3.3. Exponential Smoothing (Prophet/ARIMA Proxy)
    try:
        hw_model = ExponentialSmoothing(y_daily, trend='add', seasonal='add', seasonal_periods=7)
        hw_fit = hw_model.fit()
        pred_hw = hw_fit.forecast(remaining_days)
    except:
        pred_hw = np.full(remaining_days, y_daily[-7:].mean())
        
    # Ensemble Average
    ensemble_daily = (pred_mlp * 0.4) + (pred_gbr * 0.2) + (pred_hw * 0.4)
