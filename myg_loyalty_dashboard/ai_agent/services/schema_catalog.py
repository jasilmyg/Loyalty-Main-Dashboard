...
class SchemaCatalogService:
    ...
    TABLE_DESCRIPTIONS = {
        "sales_data": "Raw individual sales transaction rows. Columns: parsed_date (date), customer_mobile (text, unique customer ID), total_value (numeric, sale amount), branch (text), customer_type (text: New/Repeat). Use THIS table when the user asks for cross-year customer cohort comparisons such as 'customers who purchased in year X but not year Y'. Use COUNT(DISTINCT \"customer_mobile\") with NOT EXISTS subqueries to find such cohorts. EXAMPLES:\nQ: dormant customers from 2024 who came back in 2026?\nA: ```sql\nWITH cohort24 AS (SELECT \"customer_mobile\" FROM v_sales_data WHERE \"date\">='2024-01-01' AND \"date\"<'2025-01-01'), cohort26 AS (SELECT \"customer_mobile\" FROM v_sales_data WHERE \"date\">='2026-01-01' AND \"date\"<'2027-01-01') SELECT COUNT(DISTINCT a.\"customer_mobile\") FROM cohort26 a INNER JOIN cohort24 b ON a.\"customer_mobile\" = b.\"customer_mobile\";\n```\n\nQ: unique customer count whose purchase in 2024 but not purchase in 2026\nA: ```sql\nSELECT COUNT(DISTINCT sd.\"customer_mobile\") AS unique_customer_count FROM sales_data sd WHERE EXTRACT(YEAR FROM sd.parsed_date) = 2024 AND NOT EXISTS (SELECT 1 FROM sales_data sd2 WHERE sd2.\"customer_mobile\" = sd.\"customer_mobile\" AND EXTRACT(YEAR FROM sd2.parsed_date) = 2026);\n```\n\nQ: how many customers bought in 2023 but stopped in 2024?\nA: ```sql\nSELECT COUNT(DISTINCT sd.\"customer_mobile\") AS unique_customer_count FROM sales_data sd WHERE EXTRACT(YEAR FROM sd.parsed_date) = 2023 AND NOT EXISTS (SELECT 1 FROM sales_data sd2 WHERE sd2.\"customer_mobile\" = sd.\"customer_mobile\" AND EXTRACT(YEAR FROM sd2.parsed_date) = 2024);\n```",
        ...
        "mv_dormant_reactivation": "Pre-aggregated dormant customer counts. Columns: cohort_year (numeric), first_2026_month (timestamp, NULL if no 2026 purchase), unique_customers (integer, count of customers). Use to find count of customers whose last purchase was in a specific year (filter using cohort_year) and did not purchase in 2026 (filter using first_2026_month IS NULL). When counting, always use SUM(unique_customers).",
        "mv_branch_resurrection_2024_2026": "Branch-level resurrection metrics for 2024 cohort returning in 2026. Actual columns: branch_name (text), resurrected_customers (integer), total_2024_customers (integer), resurrection_rate (numeric). Do NOT use unique_customers or date_column — they do not exist in this view.",
