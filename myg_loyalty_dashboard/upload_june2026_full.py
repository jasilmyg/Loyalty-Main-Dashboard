...
# --- STEP 2: Delete existing June 2026 data -------------------------------------
print("\n" + "=" * 60)
print("STEP 2: Deleting existing June 2026 data...")
with connection.cursor() as cur:
    # Delete where parsed_date is in June 2026. If parsed_date is null, this might not catch it,
    # but let's delete based on parsed_date.
    cur.execute("DELETE FROM sales_data WHERE parsed_date >= '2026-06-01' AND parsed_date < '2026-07-01';")
    june_deleted = cur.rowcount
    print(f"  -> Deleted {june_deleted} rows from June 2026.")
    connection.commit()

# --- STEP 3: Read Excel file ----------------------------------------------------
print("\n" + "=" * 60)
print("STEP 3: Reading Excel file...")
file_path = r"C:\Users\jasil_myg\Desktop\myG Loyalty Main Dashboard\project_folder\DSR JUNE 2026 FULL.xlsx"
df = pd.read_excel(file_path, engine='calamine')
original_count = len(df)
print(f"Loaded {original_count} rows from Excel.")
print(f"Excel columns: {df.columns.tolist()}")

# --- STEP 4: Filter out SMC/EI and HEAD OFFICE / UG SMART CHOICE --------------
print("\n" + "=" * 60)
print("STEP 4: Filtering out bad data from Excel before insert...")
...
# --- STEP 5: Align columns with DB ---------------------------------------------
print("\n" + "=" * 60)
print("STEP 5: Aligning Excel columns with DB schema...")
...
# --- STEP 6: Check for duplicate Invoice Numbers already in DB ------------------
print("\n" + "=" * 60)
...
print("\n" + "=" * 60)
print(f"STEP 8: Inserting {len(df_clean)} clean rows into PostgreSQL...")
...
# --- STEP 9: Delete SMC/EI and bad branches from ENTIRE DB ---------------------
print("\n" + "=" * 60)
print("STEP 9: Removing SMC/EI and HEAD OFFICE / UG SMART CHOICE from ENTIRE DB...")
...
with connection.cursor() as cur:
    ...
    # also make sure parsed_date is up to date for new entries
    cur.execute("""
        UPDATE sales_data
        SET parsed_date = CAST("Date" AS DATE)
        WHERE parsed_date IS NULL AND "Date" IS NOT NULL;
    """)
    print(f"  -> Updated {cur.rowcount} rows with parsed_date.")

connection.commit()
print("  + Deletions and updates committed.")

# --- STEP 10: Verify final count ------------------------------------------------
print("\n" + "=" * 60)
...
with connection.cursor() as cur:
    ...
    print(f"  -> Last parsed_date in DB: {last_date}")

# --- STEP 11: Refresh materialized views ---------------------------------------
print("\n" + "=" * 60)
print("STEP 11: Refreshing materialized views...")
import subprocess
# I'll run fast_refresh_all.py to refresh all 26 views concurrently
result = subprocess.run([sys.executable, 'fast_refresh_all.py'], capture_output=True, text=True)
...
print("\n" + "=" * 60)
print("✅ ALL DONE!")
print(f"   Deleted (June 2026 initially): {june_deleted}")
